...
which are both assumed to depend on age $a$.  Remission is also possible for some diseases, with rate $r(a)$, though for clarity it is not illustrated the examples here.  Assume further that the rates are constant within integer years of age $a$, so they can be written $i_a$, $f_a$.
...
2. _risk probability estimate with denominator_: through arguments `inc` and `inc_denom`.  The numerator is computed automatically, by multiplying the denominator by the risk estimate. 

3. _estimate with lower and upper 95% credible limits_, through arguments `inc`, `inc_lower` and `inc_upper`. The numerator and denominator are computed automatically, as described above. 
