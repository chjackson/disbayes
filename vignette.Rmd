...
This approach is used in the DisMod-MR software, as explained by @flaxman2015, however the software itself is undocumented and not fully published.  The older (published) DisMod II (@dismod2) used an optimisation approach to estimate parameters, which is a less statistically-principled rough approximation.  Other advantages of the Bayesian method include 
...
The first few rows of the full data frame are shown here.  The estimates from DisMod II are given in the variable `cfout`.  The other variables are explained below.
...
```
In this example, the theoretical disease model, statistical model and priors are written down in Stan language in the file [gbdcf-unsmoothed.stan](gbdcf-unsmoothed.stan).
