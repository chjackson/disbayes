...
data {
    ...
    int<lower=0> K; // number of spline basis variables excluding the intercept
    matrix[nage,K] X;
}
...
//    print(target());
    ...
    // Whether or not to use supplied prevalence data?
    // should be roughly same as prevalence forward-calculated from incidence
}
