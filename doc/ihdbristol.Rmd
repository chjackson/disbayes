...
2. write down a statistical model for the observed data given the parameters of the underlying disease model.    In this model, observed data need to be expressed as counts of individuals and associated denominators, e.g. 
  ...
  * (incidence): $n_a^{(inc)}, r_a^{(inc)}$: given a population of size $n_a$^{(mort)}, $r_a^{(inc)}$ of these are observed to get the disease within the next year 
   
  * (mortality): $n_a^{(mort)}, r_a^{(mort)}$: given a population of size $n_a^{(mort)}$ (with or without the disease), $r_a^{(mort)}$ of these are observed to die from the disease within the next year 
  
  * (prevalence): $n_a^{(prev)}, r_a^{(prev)}$: from a sample of $n_a^{(prev)}$ individuals, $r_a^{(prev)}$ are known to have the disease at age $a$ (and $n_a^{(prev)} - r_a^{(prev)} are known to not have the disease. 
...
The `disbayes` package uses Stan to fit the Bayesian disease model given here to the data $\mathbf{y} = \{i_a, r_a, n_a\}$ supplied in the `ihdbristol` data frame.   The function `disbayes` does the work of converting the data to the required format for Stan, and initializing and a set of Markov Chain Monte Carlo simulations, and calling the Stan software to run the simulations.
